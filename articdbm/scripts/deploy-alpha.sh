#!/usr/bin/env bash
# =============================================================================
# articdbm Alpha Deployment Script
...
#   NAMESPACE             Target namespace (default: articdbm-alpha)
#   APP_HOST              Application hostname (default: articdbm.localhost.local)
#
# =============================================================================
...
readonly APP_NAME="${APP_NAME:-articdbm}"
readonly KUBE_CONTEXT="${KUBE_CONTEXT:-local-alpha}"
readonly NAMESPACE="${NAMESPACE:-articdbm-alpha}"
readonly APP_HOST="${APP_HOST:-articdbm.localhost.local}"
readonly OVERLAY_PATH="${OVERLAY_PATH:-k8s/kustomize/overlays/alpha}"

# Services with Dockerfiles (customize per repo)
declare -a SERVICES=("manager" "proxy")
