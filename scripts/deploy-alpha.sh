...
#   NAMESPACE             Target namespace (default: nest)
#   APP_HOST              Application hostname (default: nest.localhost.local)
#
# =============================================================================
...
readonly APP_NAME="${APP_NAME:-nest}"
readonly KUBE_CONTEXT="${KUBE_CONTEXT:-local-alpha}"
readonly NAMESPACE="${NAMESPACE:-nest}"
readonly APP_HOST="${APP_HOST:-nest.localhost.local}"
readonly OVERLAY_PATH="${OVERLAY_PATH:-k8s/kustomize/overlays/alpha}"

# Services to build and deploy
declare -a SERVICES=("api" "manager" "web")

# Service-to-source-path mapping (relative to PROJECT_ROOT)
# Used for Dockerfile discovery and existence checks
...
# Service-to-build-context mapping (relative to PROJECT_ROOT)
# Use "." (repo root) for services whose Dockerfile needs the full repo (e.g., Go modules)
...
build_and_import() {
    local service="$1"
    local tag="$2"
    local rel_path="${SERVICE_PATHS[${service}]}"
    local service_path="${PROJECT_ROOT}/${rel_path}"

    if [[ ! -d "${service_path}" ]]; then
        print_warning "Service directory not found: ${rel_path} — skipping"
        return 0
    fi
    ...
    # Resolve build context: use SERVICE_CONTEXTS if defined, else fall back to service_path
    local rel_context="${SERVICE_CONTEXTS[${service}]:-${rel_path}}"
    local build_context="${PROJECT_ROOT}/${rel_context}"

    local image_name="${SERVICE_IMAGES[${service}]}:${tag}"

    print_info "Building image: ${image_name} (context: ${rel_context}, dockerfile: ${rel_path}/$(basename "${dockerfile}"))"
    if ! docker build \
        --file "${dockerfile}" \
        --tag "${image_name}" \
...
OPTIONS:
    ...
    --service SERVICE     Build specific service only (e.g., api, manager, web)
