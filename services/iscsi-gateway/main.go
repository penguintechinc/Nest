...
func main() {
	...
	// Target management API — require Bearer JWT token with tenant claim
	targets := r.Group("/api/v1/targets")
	targets.Use(authMiddleware()) // Apply auth to all target endpoints
...
func authMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// For now, accept any non-empty Bearer token and extract tenant from request body (CreateTarget)
		// or query param (ListTargets/GetTarget).
